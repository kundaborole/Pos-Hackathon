          ...
          <div className="bg-white border border-border-warm rounded-2xl p-6 shadow-sm text-center mt-2">
            {submitted ? (
              <div className="animate-in fade-in zoom-in duration-300">
                ...
                <h3 className="font-bold text-text-primary mb-1">Thank you!</h3>
                <p className="text-xs text-text-secondary">Your feedback helps us improve.</p>
              </div>
            ) : (
              <>
                ...
                <div className="flex justify-center space-x-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button 
                      ...
                      className="w-10 h-10 rounded-full flex items-center justify-center border-2 border-transparent transition-all active:scale-95"
                    >
                      <Star 
                        className={`h-7 w-7 transition-colors ${
                          (hoverRating || rating) >= star 
                            ? "fill-muted-gold text-muted-gold" 
                            : "fill-transparent text-border-warm hover:text-muted-gold/50"
                        }`} 
                      />
                    </button>
                  ))}
