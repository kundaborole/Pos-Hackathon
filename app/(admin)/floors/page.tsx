...
export default function FloorsConfigurationPage() {
  ...
  const activeFloorTables = MOCK_TABLES.filter(t => t.floorId === activeFloorId);
        ...
        <div className="w-full md:w-64 flex flex-col space-y-2 bg-bg-surface p-4 rounded-xl border border-border-warm shrink-0">
          ...
          {MOCK_FLOORS.map(floor => (
            ...
            >
              ...
              {!floor.active && <span className="text-[10px] uppercase bg-bg-secondary px-1.5 py-0.5 rounded text-text-secondary">Inactive</span>}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 w-full space-y-4">
          <div className="flex justify-between items-center bg-bg-surface p-4 rounded-md border border-border-warm">
            <h3 className="font-semibold text-text-primary">
              Tables on {MOCK_FLOORS.find(f => f.id === activeFloorId)?.name}
            </h3>
              ...
              <TableBody>
                {activeFloorTables.map((table) => (
                  <TableRow key={table.id}>
                    <TableCell className="font-medium text-text-primary">T-{table.number}</TableCell>
                    <TableCell className="text-text-secondary">{table.seats}</TableCell>
                    <TableCell>
                      <StatusBadge status={table.active ? "success" : "inactive"} label={table.active ? "Active" : "Inactive"} />
                    </TableCell>
                    <TableCell>
        ...
        <div className="space-y-4 pt-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input defaultValue={editingTable?.number || ""} placeholder="e.g. 12" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-text-primary">Seats</label>
              <Input type="number" defaultValue={editingTable?.seats || 4} />
            </div>
          </div>
          
          <div className="space-y-2">
            <label className="text-sm font-medium text-text-primary">Floor</label>
            <Select defaultValue={editingTable?.floorId || activeFloorId}>
          ...
          <div className="flex items-center justify-between py-2 border-b border-border-warm">
            ...
            <Switch checked={editingTable ? editingTable.active : true} readOnly />
          </div>
