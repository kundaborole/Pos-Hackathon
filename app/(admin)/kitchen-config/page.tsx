...
export default function KitchenConfigPage() {
  const [stations] = React.useState(MOCK_KITCHEN_STATIONS);
  const [isModalOpen, setIsModalOpen] = React.useState(false);
  const [editingStation, setEditingStation] = React.useState<KitchenStation | null>(null);
      ...
      >
        <div className="space-y-4 pt-4">
          <div className="space-y-2">
            ...
            <Input defaultValue={editingStation?.name || ""} placeholder="e.g. Grill Station" />
          </div>
          
          <div className="space-y-2">
            <label className="text-sm font-medium text-text-primary">Assigned Categories (Comma separated)</label>
            <Input defaultValue={editingStation?.categories.join(", ") || ""} placeholder="e.g. Burgers, Steaks" />
          </div>
          ...
          <div className="flex justify-end space-x-2 pt-4 border-t border-border-warm mt-4">
            ...
            <Button onClick={handleCloseModal}>Save Station</Button>
          </div>
        </div>
      </Modal>
    </div>
